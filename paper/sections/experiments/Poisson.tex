...
\begin{table}[H]
    ...
    \begin{tabular}{>{\centering\arraybackslash}p{1.4cm}*{10}{>{\centering\arraybackslash}p{.8cm}}}
        ...
        \hline
    \end{tabular}
    \caption{MPDenoiseNet - Poisson Detailed PSNR Results}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\centering\arraybackslash}p{1.4cm}*{10}{>{\centering\arraybackslash}p{.8cm}}}
        ...
        \hline
    \end{tabular}
    \caption{MPDenoiseNet - Poisson Detailed SSIM Results}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{ccccc}
        ...
        \hline
    \end{tabular}
    \caption{Poisson Noise MPDenoiseNet vs. Restormer Comparison}
\end{table}

In the task of Poisson noise removal, MPDenoiseNet demonstrates a significant advantage over Restormer in terms of average PSNR, consistently achieving higher scores across all tested noise densities (150 to 240). Conversely, Restormer consistently obtains slightly better average SSIM results compared to MPDenoiseNet under these conditions. This indicates that while MPDenoiseNet excels in pixel-level accuracy (PSNR) for Poisson denoising, Restormer maintains a marginal edge in preserving structural similarity (SSIM).