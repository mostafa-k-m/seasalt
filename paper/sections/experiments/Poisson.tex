...
\begin{table}[H]
    ...
    \begin{tabular}{>{\centering\arraybackslash}p{1.4cm}*{10}{>{\centering\arraybackslash}p{.8cm}}}
        ...
        \hline
    \end{tabular}
    \caption{MPDNet - Poisson Detailed PSNR Results}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\centering\arraybackslash}p{1.4cm}*{10}{>{\centering\arraybackslash}p{.8cm}}}
        ...
        \hline
    \end{tabular}
    \caption{MPDNet - Poisson Detailed SSIM Results}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{ccccc}
        ...
        \hline
    \end{tabular}
    \caption{Poisson Noise MPDNet vs. Restormer Comparison}
\end{table}

In the task of Poisson noise removal, MPDNet demonstrates a significant advantage over Restormer in terms of average PSNR, consistently achieving higher scores across all tested noise densities (150 to 240). Conversely, Restormer consistently obtains slightly better average SSIM results compared to MPDNetunder these conditions. This indicates that while SMPDNetexcels in pixel-level accuracy (PSNR) for Poisson denoising, Restormer maintains a marginal edge in preserving structural similarity (SSIM).